...
import { getClinicDoctors } from "@/shared/api/clinic-cabinet/requests";
...
export const ClinicSpecialistsPage: FC = () => {
  ...
  const specialists: Specialist[] = (doctorsData?.data ?? []).map((d) => ({
    id: String(d.id),
    name: d.full_name,
    specialty: d.specialty,
    clinic: profile?.name ?? "",
    rating: d.rating,
    reviews: d.appointments_total,
    experience: 0,
  ...
  return (
    <div className="w-full min-h-screen">
      <div className="md:hidden flex items-center justify-between px-4 py-4 bg-white border-b border-[#E5E6E8]">
        <h1 className="text-lg font-semibold text-[#191A1B]">Мои специалисты</h1>
        <Link href="/clinic-profile/specialists/new" className="px-4 py-2 rounded-full bg-[#F5653E] text-white text-sm font-medium hover:bg-[#E5542D] transition-colors">
          Добавить
        </Link>
      </div>
        ...
        <div className="flex gap-6">
          ...
          <main className="flex-1 min-w-0">
            <div className="hidden md:flex items-center justify-between mb-6">
              <h2 className="text-[32px] font-semibold text-[#191A1B]">Мои специалисты</h2>
              <Link href="/clinic-profile/specialists/new" className="px-6 py-3 rounded-full bg-[#F5653E] text-white font-medium hover:bg-[#E5542D] transition-colors whitespace-nowrap">
                Добавить нового
              </Link>
            </div>
            <SpecialistsList specialists={specialists} onDelete={() => {}} />
          </main>
        </div>
      </div>
    </div>
