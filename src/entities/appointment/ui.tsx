    ...
    <>
        ...
        <div className="flex-1 min-w-0 pt-1">
          ...
          <div className="flex flex-col gap-2.5 text-base text-[#686F72]">
            <div className="flex items-center gap-2">
              <CalendarIcon className="w-5 h-5 shrink-0 [&_path]:stroke-[#686F72]" />
              <span>
      ...
      <div className="md:hidden bg-white rounded-3xl p-4 border border-[#E5E6E8]">
        <div className="flex items-start gap-3 mb-3">
          <div className="relative shrink-0">
            <div className="relative w-20 h-20 rounded-2xl overflow-hidden bg-[#FFF8F5]">
              {appointment.doctorImage ? (
                <Image
                  src={appointment.doctorImage}
                  alt={appointment.doctorName}
            ...
            <div className="absolute top-2 left-2 flex items-center gap-0.5 bg-white rounded-md px-1.5 py-0.5 shadow-sm">
              <StarIcon className="w-3 h-3 text-[#F5653E]" />
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-[#191A1B] font-semibold text-base leading-tight mb-0.5">
              {appointment.doctorName}
            </h3>
            ...
            <div className="flex flex-col gap-1.5 text-sm text-[#686F72]">
              <div className="flex items-center gap-1.5">
                <CalendarIcon className="w-4 h-4 shrink-0 [&_path]:stroke-[#686F72]" />
                <span>
                  {appointment.date} • {appointment.time}
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <MedicalServiceIcon className="w-4 h-4 shrink-0 [&_path]:stroke-[#686F72]" />
                <span>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 text-sm text-[#686F72] flex-1 min-w-0 bg-[#F2F3F5] px-3 py-2 rounded-xl">
            <GeoIcon className="w-4 h-4 shrink-0 [&_path]:stroke-[#686F72]" />
            <span className="flex-1 truncate">{appointment.address}</span>
          </div>
          {appointment.status === "upcoming" && onCancel && (
            <button
              onClick={() => onCancel(appointment.id)}
              className="w-9 h-9 rounded-full border border-[#E5E6E8] flex items-center justify-center shrink-0 hover:bg-[#FFF8F5] transition-colors"
              aria-label="Отменить"
            >
