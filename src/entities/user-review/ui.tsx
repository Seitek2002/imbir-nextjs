      ...
      case "clinic":
        return (
          <div className="flex items-center gap-1 text-[#838A8D] text-sm mt-0.5">
            <GeoIcon className="w-4 h-4 [&_path]:stroke-[#838A8D] shrink-0-0" />
    ...
    <div className="bg-white rounded-3xl p-6 border border-[#E5E6E8] flex flex-col gap-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          {review.image ? (
            <div className="w-12 h-12 rounded-full overflow-hidden shrink-0-0">
              <Image
                src={review.image}
                alt={getTitle() || ""}
                width={48}
            ...
            <div className="w-12 h-12 rounded-full bg-[#F8F9FA] flex items-center justify-center text-[#838A8D] text-lg font-semibold shrink-0-0">
              {getTitle()?.charAt(0) || "?"}
            </div>
          )}
        ...
        <div className="flex items-center gap-2 shrink-0-0">
          {onEdit && (
            <button
