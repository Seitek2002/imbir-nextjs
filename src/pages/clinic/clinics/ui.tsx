...
//
// Специализация тоже уходит на бэк, но там БАГ: /api/clinics/ фильтрует по
// одному значению (`name__iexact` по всей строке), а фильтр у нас
// множественный и шлёт «А,Б» — бэк ищет специализацию с таким именем целиком
// и не находит ничего. Проверено живыми запросами: без фильтра 6 клиник,
// `?specialization=Кардиология` — 1, `?specialization=Кардиология,Неврология`
// — 0. У /api/doctors/ тот же параметр уже умеет список через запятую (OR),
// так что фронт шлёт правильно — ждём, пока клиники подтянут к тому же виду.
